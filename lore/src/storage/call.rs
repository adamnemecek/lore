...
///    or already closed, emits `LORE_EVENT_ERROR` + `Complete{status:1}`
///    and returns `1` without invoking the op impl.
    ...
    Fut: Future<Output = Result<ResT, ErrT>> + 'static,
{
    ...
    LORE_CONTEXT
        .scope(execution, async move {
            ...
            let status = match command(store, args).await {
                Ok(_) => 0,
                Err(err) => {
                    execution_context().dispatcher.send_error(err);
                    1
...
mod tests {
    ...
    /// Minimal record of an emitted event preserving the values tests
    /// care about: discriminant for kind, and the `Complete.status` for
    /// ordering/value assertions.
    ...
    #[tokio::test]
    async fn op_error_emits_error_and_completes_with_status_one() {
        let (store, store_handle) = register_test_store().await;
        let sink: Arc<Mutex<Vec<CapturedEvent>>> = Arc::new(Mutex::new(Vec::new()));
        let status = storage_call(
            LoreGlobalArgs::default(),
